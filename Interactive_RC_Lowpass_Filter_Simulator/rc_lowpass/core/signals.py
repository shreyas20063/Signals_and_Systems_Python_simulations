...
def bode_response(rc_seconds: float, num_points: int = 1000) -> Tuple[np.ndarray, np.ndarray]:
    ...
    f_min = 10 ** settings.BODE_FREQUENCY_DECADE_RANGE[0]
    f_max = 10 ** settings.BODE_FREQUENCY_DECADE_RANGE[1]
...
def square_wave_harmonics(fundamental_hz: float, amplitude_v: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    odd_indices = np.arange(1, settings.MAX_HARMONIC_ORDER + 1, 2)
...
def cutoff_frequency(rc_seconds: float = settings.DEFAULT_RC_SECONDS) -> float:
