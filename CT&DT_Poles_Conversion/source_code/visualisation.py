...
class PlotRenderer:
    """Handles all plot rendering and visualization"""
    ...
    def _configure_s_plane_display(self):
        ...
        # Title with spacing
        self.ax_s_plane.set_title("S-Domain Analysis", pad=16, fontsize=11, fontweight='bold')
        
        # Legend outside the axes so it doesn’t overlap poles/zeros
        self.ax_s_plane.legend(
            loc='upper right',
            fontsize=9,
            framealpha=0.9,
            bbox_to_anchor=(1.9, 0.75),
            borderaxespad=0
        )
    ...
    # Expand limits a bit for breathing room
        ...
        self.ax_z_plane.legend(loc='upper left', framealpha=0.9, fontsize=9)
        ...
        self.ax_z_plane.legend(
            loc='upper left', fontsize=9, framealpha=0.9,
            bbox_to_anchor=(1.02, 1.0), borderaxespad=0
        )
        
        # Keep circular aspect ratio
        self.ax_z_plane.set_aspect('equal')
...
class UserInterface:
    ...
    def __init__(self, figure, explorer_instance):
        self.fig = figure
        self.explorer = explorer_instance
        self.setup_interactive_controls()
    
    def setup_interactive_controls(self):
        """Setup intuitive controls for classroom use"""
        
        # Main T/τ control with clear labeling
        ax_slider = plt.axes([0.15, 0.08, 0.35, 0.03])
        ax_slider.set_facecolor(Config.COLORS['background'])
        self.slider_T_tau = Slider(ax_slider, 'Step Size Ratio (T/τ)', 0.01, 3.0, 
                                  valinit=0.5, valfmt='%.2f', 
                                  color=Config.COLORS['accent'])
        self.slider_T_tau.on_changed(self.explorer.update_step_size)
        
        # Method selection with clear descriptions
        ax_method = plt.axes([0.72, 0.15, 0.18, 0.15])
        ax_method.set_facecolor(Config.COLORS['background'])
        self.radio_method = RadioButtons(ax_method, ('Forward Euler', 'Backward Euler', 'Trapezoidal'),
                                        active=0, activecolor=Config.COLORS['accent'])
        self.radio_method.on_clicked(self.explorer.change_method)
        
        # Learning mode controls
        self._setup_learning_controls()
    
    def _setup_learning_controls(self):
        """Setup learning mode control buttons"""
        ax_reset = plt.axes([0.75, 0.08, 0.06, 0.04])
        self.btn_reset = Button(ax_reset, 'Reset', color=Config.COLORS['stable_discrete'])
        self.btn_reset.on_clicked(self.explorer.reset_to_defaults)
        
        ax_animate = plt.axes([0.83, 0.08, 0.06, 0.04])
        self.btn_animate = Button(ax_animate, 'Demo', color='lightblue')
        self.btn_animate.on_clicked(self.explorer.run_demo_animation)
